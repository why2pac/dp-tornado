...
class SetController(Controller):
    def get(self, key, val, expire_in=None, delay=None):
        ...
        if not expire_in:
            self.model.tests.model_test.cache_test.set_redis(key=key, val=val)
        else:
            self.model.tests.model_test.cache_test.set_redis_with_expire(key=key, val=val, expire_in=expire_in)
        ...
        if delay and expire_in > 0:
            ...
            cached_after_expire = self.model.tests.model_test.cache_test.get_redis(key=key)

            assert cached_after_expire is None

            return self.finish('cache-redis:%s=%s=>%s' % (key, cached, cached_after_expire or 'empty'))

        self.finish('cache-redis:%s=%s' % (key, cached))
